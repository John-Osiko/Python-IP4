...
class CommentModelTest(unittest.TestCase):
    def setUp(self):
        ...
        self.new_comment = Comment(id = 1, comment = 'Test comment', user = self.user_tripper, blog_id = self.new_blog)
    ...
    def test_check_instance_variables(self):
        ...
        self.assertEquals(self.new_comment.user,self.user_tripper)
        self.assertEquals(self.new_comment.blog_id,self.new_blog)


class CommentModelTest(unittest.TestCase):
    def setUp(self):
        self.user_john = User(username='john', password='biski', email='test@test.com')
        self.new_blog = Blog(id=1, title='Test', content='This is a test blog', user_id=self.user_john.id)
        self.new_comment = Comment(id=1, comment ='This is a test comment', user_id=self.user_john.id, blog_id = self.new_blog.id )
